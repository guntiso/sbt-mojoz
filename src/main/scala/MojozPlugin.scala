...
object MojozPlugin extends AutoPlugin {
  object autoImport {
    ...
    val mojozBeforeCompile = taskKey[File]("Creates or copies tresql-scala-macro.properties before compile")
  ...
  override val projectSettings: Seq[Def.Setting[?]] = Seq(
    ...
    mojozBeforeCompile := {
      val dest = (Compile / classDirectory).value / "tresql-scala-macro.properties"
      ((Compile / unmanagedResources).value ** "tresql-scala-macro.properties").get.headOption match {
        case Some(src) => IO.copyFile(src, dest)
        case None =>
