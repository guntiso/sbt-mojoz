...
object MojozPlugin extends AutoPlugin {
  object autoImport {
    ...
    val mojozShouldGenerateDtoImplied = settingKey[Boolean]("Should DTO be genrated for view if not explicitly stated in dto field of that view, defaults to true")
  ...
  override val projectSettings: Seq[Def.Setting[?]] = Seq(
    ...
    mojozShouldGenerateDtoImplied := true,
    mojozGenerateDtosViewMetadata := Def.uncached {
      val implied = mojozShouldGenerateDtoImplied.value
      val allViewDefs = mojozViewMetadata.value
      val nameToViewDef = allViewDefs.map(v => v.name -> v).toMap
      def shouldGenerateDto(viewDef: ViewDef): Boolean =
        Option(viewDef.extras).flatMap(_.get("dto")).map {
          case b: java.lang.Boolean     => b.booleanValue
          case b: Boolean               => b
          case s: String if s == "dto"  => true
          case s: String                => s.toBoolean
          case x => sys.error(
            s"Expecting boolean value for dto extra, view: ${viewDef.name}, got: ${Option(x).map(_.getClass.getName).orNull}")
        }.getOrElse(implied)
      val selected = allViewDefs.filter(shouldGenerateDto)
      val names = collection.mutable.LinkedHashSet.empty[String]
      val queue = collection.mutable.Queue.empty[ViewDef]
      selected.foreach { v =>
        if (names.add(v.name)) queue.enqueue(v)
      }
      while (queue.nonEmpty) {
        val v = queue.dequeue()
        v.fields.foreach { f =>
          if (f.type_ != null && f.type_.isComplexType && names.add(f.type_.name))
            nameToViewDef.get(f.type_.name).foreach(queue.enqueue(_))
        }
        if (v.extends_ != null && names.add(v.extends_))
          nameToViewDef.get(v.extends_).foreach(queue.enqueue(_))
      }
      // Preserve original name-sorted order of plainViewDefs among selected names
      allViewDefs.filter(v => names.contains(v.name))
    },
    mojozGenerateDtosMappingsViewMetadata := Def.uncached(mojozGenerateDtosViewMetadata.value),
